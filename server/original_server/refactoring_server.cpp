#include "refactoring_server.hpp"
#include <iostream>
#include <sstream>
#include <fstream>
#include <algorithm>
...
void Server::handleGetRequest(int clientSocket, ServerConf &conf) {
    ...
    if (isValidHost(conf)) {
        if (ext.empty() || (ext != "sh" && ext != "py" && ext != "pl"
            ...
            if (isDirectory(file.c_str()) || file.empty()) {
...
std::string Server::findDirFile(std::string &file, const ServerConf &conf) {
    ...
    while ((entry = readdir(dir))) {
        ...
        if (it != conf.locationConstEnd()) {
            if (!it->lo_indexes.empty()) {
                std::vector<std::string>::const_iterator index = std::find(it->lo_indexes.begin(),
                                                                           it->lo_indexes.end(), name);
                if (index != it->lo_indexes.end()) {
                    response = getHTTPCode("200", file + *index);
                    break ;
                }
            }
        }
    ...
    if (it != conf.locationConstEnd() && !it->try_files.second.empty()) {
        ...
        while ((entry = readdir(dir))) {
            ...
            if (altFile != it->try_files.second.end()) {
                response = getHTTPCode("200", dirName + *altFile);
                break ;
            }
        }
        ...
        if (response.empty()) {
            ...
            response = getHTTPCode(code.str());
        }
        closedir(dir);
    } else {
        //TODO defErrorPage
        response = getHTTPCode(getDefErrorPage(404, conf));
...
std::string Server::getDefErrorPage(unsigned short code, const ServerConf &conf) {
    std::string file;

    ServerConf::UshortVecMap::const_iterator it = conf.server().defErrorPage.find(code);
    if (it != conf.server().defErrorPage.end()) {
        DIR *dir;
        if (conf.server().rootDir.empty()) {
            dir = opendir(".");
        } else {
            dir = opendir(conf.server().rootDir.c_str()); //TODO test, not working
        }
        ...
        if (dir != NULL) {
            ...
            while ((entry = readdir(dir)) != NULL) {
                ...
                ServerConf::StrVector::const_iterator it2 = std::find(it->second.begin(), it->second.end(), name);
                if (it2 != it->second.end()) {
                    file = it2->substr(0, it2->find_last_of('.'));
                    break;
                }
            }
            closedir(dir);
