...
SocketManager::SockIter SocketManager::swapSockConf(int sockFd, const ServerConf &conf, int domain, int type) {
...
int SocketManager::listenOnSock(SockIter it) {
    ...
    if (it->second.server().ipv4Addr.sin_addr.s_addr < UINT_MAX) {
        ...
        if (listen(it->first, 10) == -1) { //TODO el numero 10 == kk, aprender a definir este num
